...
const load = url => cheerio.load(request('GET', url).getBody().toString())
  ...
  const listeAuslesen = (nr, schulart, spalte, einspaltig) =>
    $('#content-main ul')
      ...
      .flatMap(a => {
        ...
        return parsePage(a.link, schulart, spalte, einspaltig).flatMap(b => ({
          ...b,
          schulart: schulart,
          fach: a.fach,
...
const parsePage = (url, schulart, spalte, einspaltig) => {
  ...
  if (
    $('h1.basecontent-pbsbw---headline-h1')
      .text()
      .match(/Fachpläne/)
  )
    return parsePage(baseUrl + $('.slot-content a[href]').eq(1).attr('href'))
  else
    return (
      $('.tableGrid.hide-grid')
          ...
          return $(this)
            ...
            .map(function () {
              ...
              return {
                ...
                link: hatTeilkompetenzen ? baseUrl + $(this).attr('href') : url
              }
            })
            .get()
        ...
        .flatMap(a => {
          if (a.link) {
            const { beschreibung, teilkompetenzen } = parseDetail(
              a.link,
              schulart,
              spalte,
              einspaltig
            ...
            return [
              ...
              ...teilkompetenzen.map(b => ({
                ...
                titel: '',
                beschreibung:
                  'Die Schülerinnen und Schüler können ' + b.beschreibung,
                link: a.link,
...
const parseDetail = (url, schulart, spalte, einspaltig) => {
  const $ = load(url)
  return {
    beschreibung: $('h2.basecontent-pbsbw---headline-h1')
      .parent()
      .parent()
      .find('.teilkompetenzText')
      .first()
      .text()
      .format(),
    teilkompetenzen: $('table.tkfull.tktable tbody')
      ...
      .find(
        einspaltig
          ? 'tr'
          : 'tr.ITK_GS_ZEILE, tr.ITK_GS_ZEILE_SPAN, tr.ITK_S1_ZEILE_TK_SINGLE, tr.ITK_G8_TK'
      )
      .map(function () {
        return $(this)
          .find(einspaltig ? 'td.ITK_G8_TK' : 'td')
          .filter(function () {
            return $(this).find('.teilkompetenzNumber').length
          })
          .eq(spalte)
          .map(function () {
            return {
...
fs.writeFile('bw.json', JSON.stringify(ergebnis, null, 2), 'utf8', err => {
